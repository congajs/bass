...
function Manager(definition) {
	...
	if (definition.slaveConnections) {
		this.slaves = {};
		var keys = Object.keys(definition.slaveConnections);
		for (var i = 0; i < keys.length; i++) {
			var slave = this._createClient(definition.slaveConnections[keys[i]]);
			if (slave) {
...
Manager.prototype = {
	 ...
	 * @private
	 */
	_createClient: function(connection) {
	...
	getReaderClient: function(name) {
		if (!this.slaves) {
			...
			var keys = Object.keys(this.slaves);
			if (keys.length === 0) {
				// we don't have any slaves, return the client
				return this.client;
	 ...
	 * @return {Promise}
	 */
	findOneBy: function(name, criteria, sort){
	 ...
	 * @return {Void}
	 */
	refreshDocument: function(document, cb) {
		cb();
	},
