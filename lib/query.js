...
class Query {
    ...
    /**
     * Get all the supported operators
     * @returns {string[]}
     */
    static get operators() {
        return ['gt','gte','lt','lte','ne','in','nin','all','regex','size','maxDistance'];
    }

    /**
     * See if a string is an operator
     * @param {String} operator The operator you are checking for
     * @returns {boolean}
     */
    static isOperator(operator) {
        return this.operators.indexOf(operator.replace(/^\$/, '')) !== -1;
    }
...
Query.operators.forEach(function (conditional) {
