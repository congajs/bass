...
module.exports = class Compiler {
     ...
     * @return {void}
     */
    registerManagerDefinitions(next) {
        ...
        for (let managerName in this.config.managers) {
            ...
            const config = this.config.managers[managerName];
            ...
            calls.push(cb => {
                ...
                const definition = new ManagerDefinition();
                ...
                if (this.config.logging) {
                    ...
                    let logger = this.config.logging.logger;

                    if (config.logger && config.logger in this.config.logging) {
                        logger = this.config.logging[config.logger];
                    }
                    ...
                    if (logger) {
                        ...
                        if (typeof logger.info !== 'function' ||
                            ...
                            throw new TypeError('Logger class must contain methods: info, error, debug');
                        }
                ...
                definition.connection.boot(definition.metadataRegistry, cb);
