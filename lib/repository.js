...
module.exports = class Repository {
     ...
     * @param {String} name The collection / table name to create
     * @returns {Promise}
     */
    createCollection() {
	...
	find(id) {
		...
		// } else {
				...
				var convertedId;
				try {
					convertedId = mapper.adapterMapper.convertModelValueToDbValue(metadata.getIdField().type, id);
				} catch(e) {
					reject(e);
					return;
				}
	...
	findBy(criteria, sort = {}, skip, limit) {
		...
		return new Promise((resolve, reject) => {
			...
			this.getReaderClient().findBy(metadata, metadata.collection, dbCriteria, mappedSort, skip, limit, function(err, data){
	...
	findWhereIn(field, values, sort = {}, limit) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!Array.isArray(values) || values.length === 0) {
				...
				this.getReaderClient().findWhereIn(metadata, field, values, mappedSort, limit, function(err, data) {
					if (err) {
