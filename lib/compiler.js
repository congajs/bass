...
Compiler.prototype = {
	...
	compile: function(cb){
		...
		this.initializeConnections(function(err){
			...
			that.registerManagerDefinitions();

			cb(err, that.registry);
		});
	},

	registerManagerDefinitions: function(){
		...
		for (var i in this.config.managers){
			...
			this.registerEventListenersOnDefinition(definition);

			registry.registerManagerDefinition(i, definition);
		}
	},

	registerEventListenersOnDefinition: function(definition){
