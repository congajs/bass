...
Mapper.prototype = {
	...
	mapModelToData: function(metadata, model, cb){
		...
		this.adapterMapper.convertRelationsToData(metadata, model, data, function(doEmbeds) {
			//embeds are fields which have to be recursively mapped. sql will serialize into fields and
			//will not require mapping, but mongo will require mapping
			...
			if (doEmbeds) {
				...
				async.each(
					...
					function (embedKey, cb2) {
						var embed = metadata.embeds['one'][embedKey];
						var embedMetadata = self.registry.getMetadataByName(embed.document);
					...
					function (err) {
						...
						async.each(
							...
							function (embedKey, cb3) {
								var embed = metadata.embeds['many'][embedKey];
	 ...
	 * @param {Object} data
	 */
	reduceMappedData: function(metadata, data) {
