...
class Mapper {
    ...
    mapQueryToDatabase(metadata, query) {
        ...
        for (property in query._conditions) {
            ...
            dbQuery._conditions[converted || !name ? property : name] = this.mapQueryConditionToDatabase(
                metadata,
                property,
                query._conditions[property]
            );
        }
    ...
    /**
     * Get the metadata field object from a property value
     * @param metadata
     * @param property
     * @returns {Object}
     */
    mapMetadataField(metadata, property) {
        if (typeof this.adapterMapper.mapMetadataField === 'function') {
            return this.adapterMapper.mapMetadataField(metadata, property);
        }
        return metadata.getFieldByProperty(property);
    }

    /**
     * Map a query value to a suitable database value
     * @param metadata
     * @param property
     * @param value
     * @returns {*}
     */
    mapQueryConditionToDatabase(metadata, property, value) {
        if (typeof this.adapterMapper.convertModelValueToDbValue !== 'function') {
            return value;
        }
        const field = this.mapMetadataField(metadata, property);
        if (!field) {
            return value;
        }
        return this.mapPartialQueryConditionToDatabase(field, value);
    }

    /**
     * Map a partial query value to a suitable database value
     * @param {Object} field
     * @param {*} value
     * @returns {*}
     */
    mapPartialQueryConditionToDatabase(field, value) {
        if (value instanceof Array) {
            return value.map(val => this.mapPartialQueryConditionToDatabase(field, val));
        }
        if (value instanceof Object) {
            const clone = Object.create(value);
            let hasOperator = false;
            for (const n in value) {
                // recursively map operator values like {$in: ['id', 'id']}
                if (Query.isOperator(n)) {
                    hasOperator = true;
                    clone[n] = this.mapPartialQueryConditionToDatabase(field, value[n]);
                }
            }
            if (hasOperator) {
                return clone;
            }
            // if we did not find an operator then we did not map anything, so continue
        }
        return this.adapterMapper.convertModelValueToDbValue(field.type, value);
    }
    ...
    mapToPartialModel(metadata, data, cb) {
        ...
        // set internal flag to indicate that this is not a new instance
        Object.defineProperty(model, '__isNew', {
            value: false,
            writable: true,
            enumerable: false
        });
    ...
    mapToModel(manager, metadata, data, populate, cb, walk) {
        ...
        for (const [ idx, modelData ] of data.entries()) {

            calls.push(callback => {

                this.mapToPartialModel(metadata, modelData, (err, mappedModel) => {
        ...
        this.registry.eventDispatcher.dispatch('preHydrate', reference, function() {
            ...
            metadata.fields.forEach(function(field){
                ...
                if (value === undefined || value === null) {
                    ...
                    let defaultValue = data[field.default];
                    ...
                    switch(field.type) {
                        ...
                        case 'json' :
                        case 'object' :
                            value = defaultValue instanceof Object ? defaultValue : {};
                            break;
                ...
                if (value !== undefined) {
                    ...
                    const func = 'set' + field.property.charAt(0).toUpperCase() + field.property.substr(1);
                    if (typeof model[func] === 'function') {
            ...
            self.adapterMapper.mapPartialRelationsToModel(model, metadata, data, function(err, model) {
                ...
                metadata.fields.forEach(field => {
                    loaded[field.property] = model[field.property];
                });

                Object.defineProperty(model, '__loadedData', {
     ...
     * @param {Metadata} metadata
     * @param {Object} data
     * @param {Object} [originalData] optional unmapped data, passing this will reduce the data only to field names that exist unmapped
     */
    reduceMappedData(metadata, data, originalData = null) {

        // reduce the data given so it is suitable for storage
        const reducedData = {};

        if (originalData && !(originalData instanceof Object)) {
            originalData = null;
        }
