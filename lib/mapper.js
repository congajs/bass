...
Mapper.prototype = {
		...
		relationFields.forEach(function(field){
			...
			calls.push(
				...
				function(callback){
					...
					self.adapterMapper.convertDataRelationToDocument(metadata, field, data, model, this, function(err, document){
	...
	mapDataToModels: function(metadata, data, cb){
		...
		async.parallel(calls, function(err, mappedModels){
			//cb(err, data);
			cb(err, mappedModels);
		});
	},
