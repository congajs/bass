...
DiscriminatorListener.prototype = {
	...
	onPreHydrate: function(event, cb){
		...
		if (event.data instanceof Object &&
			...
			var document = new event.metadata.discriminator.mapped[ event.data[event.metadata.discriminator.field] ];
			...
			if (document !== event.document &&
				...
				var documentMeta = event.registry.getMetadataForDocument(document);
	...
	/**
	 * Executed when a new discriminator-document is created
	 *
	 * @param {Object} event The event object
	 * @param {Function} cb The callback
	 */
