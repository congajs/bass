...
function Manager(definition) {
	...
	/* TODO : we have to remove this - it breaks the entire flow
...
Manager.prototype = {
	...
	/**
	 * Wrap the mapper's mapDataToModel so we can persist the document after it's mapped
	 * since the mapper doesn't have a reference to our unit of work
	 *
	...
	mapDataToModel: function(metadata, document, cb, walkRecursive) {
		...
		this.mapper.mapDataToModel(
			metadata,
			document,
			// final callback
			function(err, document) {

				// persist the document after it's mapped
				self.persist(document);

				// call our final callback
				cb(err, document);
			...
			function(err, document) {
				...
				if (document) {
					// persist each embedded document
					self.persist(document);
				}

				if (typeof walkRecursive === 'function') {
					walkRecursive(err, document);
				}
			}
		);
	} ,
	...
	/**
	 * Handle mapDataToModels on the manager instead of the mapper so we can persist the document after it's mapped
	 * since the mapper doesn't have a reference to our unit of work
	 *
	 * @param {Metadata} metadata
	 * @param {[{}]} data
	...
	mapDataToModels: function(metadata, data, cb, walk) {
		...
		this.mapper.mapDataToModels(metadata, data, cb, function(err, document) {

			// recursive walker callback
