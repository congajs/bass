...
module.exports = class UnitOfWork {
    ...
    persist(document) {
        ...
        if (!document.__objectId) {
            objectId = this.setObjectId(document);
        } else {
            objectId = document.__objectId;
        }

        if (!('__processing' in document)) {
            Object.defineProperty(document, '__processing', {
                value: null,
                writable: true,
                enumerable: false
            });
        ...
        const isStrategyAuto = idStrategy === 'AUTO';
        const isStrategyManual = !isStrategyAuto && idStrategy === 'MANUAL';

        if (!('__isNew' in document)) {
            let isNew = false;
            if (isStrategyManual) {
                // this case handles documents that have a manual id, but are new
                isNew = true;
            } else if (isStrategyAuto && document[metadata.getIdPropertyName()] !== null) {
                // this case handles partial objects that aren't fully loaded and just contain an id
                isNew = false;
                Object.defineProperty(document, '__isReference', {
                    value: true,
                    writable: true,
                    enumerable: false
                });
            }
            Object.defineProperty(document, '__isNew', {
                value: isNew,
                writable: true,
                enumerable: false
            });
        ...
        if (document.__isNew ||
            (!(idField.property in document) ||
                document[idField.property] === null && isStrategyAuto)
        ) {
    ...
    clear(document) {
        ...
        if (document instanceof Object && document.__objectId) {
    ...
    /**
     * Set the object id on a document
     * @param {Object} document
     * @param {String} objectId
     * @returns {String}
     */
    setObjectId(document, objectId = null) {
        if (!objectId) {
            objectId = this.generateObjectId();
        }
        if (!('__objectId' in document)) {
            Object.defineProperty(document, '__objectId', {
                value: objectId,
                writable: true,
                enumerable: false
            });
