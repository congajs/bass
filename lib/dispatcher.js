...
EventDispatcher.prototype.dispatch = function(name, event, cb){
	...
	if ((typeof this.events[name] === 'undefined' || this.events[name].length === 0) && (typeof event.metadata.events[name] === 'undefined' || event.metadata.events[name].length === 0)){
		cb();
		return;
	}
	...
	if (typeof event.metadata.events[name] !== 'undefined' && event.metadata.events[name].length > 0) {
		for (var i in event.metadata.events[name]) {
			events.push({
				obj: event.document,
