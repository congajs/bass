...
module.exports = class Repository {
    ...
    updateBy(criteria, data) {
        ...
        let dbCriteria;
        try {
            dbCriteria = mapper.mapCriteriaToDatabase(metadata, criteria);
        } catch (e) {
        ...
        return new Promise((resolve, reject) => {
            ...
            mapper.mapModelToData(metadata, data, mappedData => {
                ...
                const reducedData = mapper.reduceMappedData(metadata, mappedData, data);
    ...
    findBy(criteria, sort = {}, skip, limit) {
        ...
        try {
            dbCriteria = mapper.mapCriteriaToDatabase(metadata, criteria);
            mappedSort = mapper.mapSortToDb(metadata, sort);
        } catch (e) {
            return Promise.reject(e);
        }
    ...
    findWhereIn(field, values, sort = {}, limit) {
        ...
        const mappedField = mapper.mapMetadataField(metadata, field);

        let mappedValues = values;

        if (mappedField && typeof mapper.adapterMapper.convertModelValueToDbValue === 'function') {
            mappedValues = values.map(value => {
                return mapper.mapPartialQueryConditionToDatabase(mappedField, value);
            });
        }

        return new Promise((resolve, reject) => {

            field = (mappedField && mappedField.name) || metadata.getFieldNameByProperty(field);
            ...
            this.getReaderClient()
                .findWhereIn(metadata, field, mappedValues, mappedSort, limit, (err, data) =>
            {
                if (err) {
                    reject(err);
                    return;
