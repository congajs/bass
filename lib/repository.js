...
Repository.prototype = {
	...
	getCollectionName: function() {
        /*if (typeof this.metadata.collection === 'function') {
	...
	updateBy: function(criteria, data) {
		...
		mapper.mapModelToData(metadata, data, function(mappedData) {
			...
			// TODO : I think this can be cleaner - make a new method for mapDataToDb or something
			if (typeof mappedData['_id'] !== 'undefined') {
				delete mappedData['_id'];
			}

			var reducedData = {};
			for (var m in mappedData) {
				if (mappedData[m] !== undefined) {
					reducedData[m] = mappedData[m];
				}
			}

			self.client.updateBy(metadata, this.getCollectionName(), dbCriteria, reducedData, function(err, result){
				...
				if (err){
					...
					documentCache.updateDocumentsByCriteria(criteria, data);
	...
	removeBy: function(criteria) {
		...
		this.client.removeBy(metadata, this.getCollectionName(), dbCriteria, function(err, result){
	...
	findBy: function(criteria, sort, skip, limit){
		...
		this.getReaderClient().findBy(metadata, this.getCollectionName(), dbCriteria, sort, skip, limit, function(err, data){
	...
	findByQuery: function(query){
		...
		this.getReaderClient().findByQuery(metadata, this.getCollectionName(), dbQuery, function(err, queryResult){
	...
	findCountByQuery: function(query){
		...
		this.getReaderClient().findCountByQuery(metadata, this.getCollectionName(), dbQuery, function(err, data) {
			if (err){
