...
Repository.prototype = {
	...
	/**
	 * Constant for the Reader client type
	 * @param {Number}
	 */
	CLIENT_TYPE_READER: 1,

	/**
	 * Constant for the Writer client type
	 * @param {Number}
	 */
	CLIENT_TYPE_WRITER: 2,
	...
	/**
	 * To force the repository to always use the writer / master client and not the slave
	 *
	 * @type {Boolean}
	 */
	_forceWriterClient: false,
	...
	/**
	 * Get a client by type, but respect the _forceWriterClient flag
	 *
	 * @param {Number|undefined} type The type of client to get (see CLIENT_TYPE constants)
 	 * @param {String|undefined} name The slave (connection name) to fetch - if not given, a random slave is returned
	 *
	 * @returns {Client}
	 */
	getClient: function(type, name) {
		if (!this._manager) {

			return null;

		} else if (!name && this._forceWriterClient) {

			return this.client;

		} else if (type === this.CLIENT_TYPE_READER) {
	...
	find: function(id){
		...
		// } else {
				...
				this.getClient(this.CLIENT_TYPE_READER).find(
					metadata,
					metadata.collection,
					convertedId,
					function(err, data) {
	...
	findBy: function(criteria, sort, skip, limit){
		...
		this.getClient(this.CLIENT_TYPE_READER).findBy(metadata, metadata.collection, dbCriteria, sort, skip, limit, function(err, data){
	 ...
	 * @return {Promise}
	 */
	findOneBy: function(criteria, sort){
	...
	findWhereIn: function(field, values, sort, limit) {
		...
		if (!Array.isArray(values) || values.length === 0) {
			...
			this.getClient(this.CLIENT_TYPE_READER).findWhereIn(metadata, field, values, sort, limit, function(err, data) {
				if (err) {
	...
	findByQuery: function(query){
		...
		this.getClient(this.CLIENT_TYPE_READER).findByQuery(metadata, metadata.collection, dbQuery, function(err, queryResult){
	...
	findCountByQuery: function(query){
		...
		this.getClient(this.CLIENT_TYPE_READER).findCountByQuery(metadata, metadata.collection, dbQuery, function(err, data) {
			if (err){
